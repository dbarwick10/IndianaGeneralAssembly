...
window.onload = async () => {
    try {
        // Fetch legislators
        const legislatorResponse = await fetch(`${url}/legislators`);
        if (!legislatorResponse.ok) throw new Error(`Error ${legislatorResponse.status}: ${legislatorResponse.statusText}`);
        const legislatorData = await legislatorResponse.json();
        
        if (legislatorData.items && legislatorData.items.length > 0) {
            legislators = legislatorData.items;
        } else {
        ...
        const billResponse = await fetch(`${url}/${currentYear}/bills`);
        if (!billResponse.ok) throw new Error(`Error ${billResponse.status}: ${billResponse.statusText}`);
        const billData = await billResponse.json();
        
        if (billData.items && billData.items.length > 0) {
            billsData = billData.items;
        } else {
        ...
        // If we still have no data, show an error message
        if ((!legislators || legislators.length === 0) && (!billsData || billsData.length === 0)) {
...
const renderAutocompleteDropdown = () => {
    ...
    dropdown.innerHTML = suggestions.map((item) => {
        if (item.type === 'legislator') {
            return `
                <li class="autocomplete-item" data-link="${item.link}" data-type="legislator">
                    ${item.fullName}
                </li>
            `;
        } else if (item.type === 'bill') {
            return `
                <li class="autocomplete-item" data-link="${item.link}" data-type="bill">
                    <div class="bill-suggestion">
                        <div class="bill-name">${item.displayName}</div>
...
// API Functions
const fetchCompleteBillData = async (link, legislatorNames) => {
    try {
        const year = document.getElementById('yearInput').value;
        const loadingElement = document.getElementById('loading');
        if (loadingElement) loadingElement.classList.remove('hidden');
        
        const response = await fetch(`${url}/${year}/bills/${link.split('/').pop()}`);
...
document.getElementById('searchInput').addEventListener('input', (e) => {
    ...
    if (lastTerm.length > 0) {
        ...
        const filteredBills = billsData.filter((bill) =>
            bill.displayName.toLowerCase().includes(lastTerm.toLowerCase()) ||
            bill.description.toLowerCase().includes(lastTerm.toLowerCase())
        );
        ...
        suggestions = [
            ...filteredLegislators.map(legislator => ({
                type: 'legislator',
                link: legislator.link,
                fullName: abbreviateTitle(legislator.fullName)
            })),
...
document.getElementById('autocompleteDropdown').addEventListener('click', (e) => {
    if (e.target.classList.contains('autocomplete-item')) {
        ...
        if (type === 'legislator') {
            ...
            const billName = e.target.querySelector('.bill-name').textContent;
            searchTerm = billName;
            document.getElementById('searchInput').value = searchTerm;
        }
...
document.getElementById('searchButton').addEventListener('click', async () => {
    ...
    const isBillSearch = searchValue.startsWith('HB') || searchValue.startsWith('SB');

    if (isBillSearch) {
        // Handle bill search
        const bill = billsData.find(b => b.displayName === searchValue);
        if (bill) {
            // Fetch complete bill data
            await fetchCompleteBillData(bill.link, []);
        } else {
            document.getElementById('noResults').classList.remove('hidden');
        }
    } else {
        // Handle legislator search (existing code)
